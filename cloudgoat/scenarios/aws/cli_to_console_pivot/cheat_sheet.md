## Cheat Sheet - cli_to_console_pivot
---
...
1. Generate a sign-in URL using the session token (this step requires proper URL encoding):

```bash

# Store session token information from the file into a variable
TOKEN=$(curl -sX PUT "http://169.254.169.254/latest/api/token" -H "X-aws-ec2-metadata-token-ttl-seconds: 21600")

```

2. Store role name using our TOKEN variable session
...
7. Generate the sign-in URL

```bash

SIGNIN_URL="https://signin.aws.amazon.com/federation?Action=getSigninToken&Session=$SESSION_URL_ENCODED"
