# EC2 Configuration for cli_to_console_pivot scenario
...
# Security group for the vulnerable EC2 instance
resource "aws_security_group" "instance_sg" {
  name        = "cg-instance-sg-${var.cgid}"
  description = "Security group for the vulnerable EC2 instance"
...
# The vulnerable EC2 instance with IMDSv2 enabled
resource "aws_instance" "vulnerable_ec2" {
  ...
  # Associate public IP for easier access (though we'll use SSM)
  associate_public_ip_address = true
  
  # Enable IMDSv2 but make it accessible
  metadata_options {
  ...
  # User data to install required tools
  user_data = <<-EOF
    #!/bin/bash
    dnf update -y
